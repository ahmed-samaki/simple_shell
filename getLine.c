	...
	if (!*len) /* If nothing left in the buffer, fill it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			...
			/* Check if there is a ';' indicating a command chain */
			if (_strchr(*buf, ';'))
			{
				*len = r;
				info->cmd_buf = buf;
...
ssize_t get_input(info_t *info)
{
	...
	*buf_p = buf; /* Otherwise, not a chain, pass back the buffer from custom_getline() */
...
int custom_getline(info_t *info, char **ptr, size_t *length)
{
	...
	if (s)
		_string_concatenate(new_p, buf + i, k - i);
	else
		_string_copy(new_p, buf + i, k - i + 1);
